This chapter establishes the technical vocabulary used throughout the document.
The proposal sits at the intersection of several areas, and each of them
contributes a concept that the later chapters assume: software analysis and
program comprehension define the tasks, static representations define what is
usually given to a model, software visualization explains why layout carries
meaning, texture descriptors and deep learning provide the two families of
methods compared in the preliminary studies, and explainability and
anonymization frame the constraints under which the approach must operate. The
goal is not to survey each area exhaustively, but to isolate the ideas and
limitations that justify treating a source file as an image.
...
organized, and what roles they play \cite{vonmayrhauser1995comprehension}.
...
architectural diagrams, and static-analysis warnings
\cite{storey2006comprehension}. These mechanisms are well established, but they
tend to operate either at the textual level or through language-aware
abstractions. A minimap-based approach adds another layer: it
...
markup, scripts, and generated artifacts. It also differs in dimensionality:
tokens and raw text are one-dimensional sequences, and trees and graphs are
relational structures without spatial layout, so the image is the only
representation in Table~\ref{tab:foundations-representations} that preserves
the two-dimensional page geometry of the artifact. Finally, it can be
anonymized in a way that removes identifiers and literal values while retaining
layout and structural symbols.
...
More recent work extends this line. FineCodeAnalyzer provides
...
inputs for computational models. The literature above produces visualization
for humans; this work develops visualization for machines, and the human
returns to the loop at the level of predictions and explanations.
...
There is, however, a difference between audio and source code that favors the
visual route even more directly. A spectrogram is a computed transform: its
two-dimensional structure is derived from a signal that has no visual layout of
its own. Source code already has one. A source file is stored as a
one-dimensional character stream, but it is written, indented, and formatted to
be displayed as a two-dimensional page, and its authors deliberately maintain
that layout. Sequence-based models discard this second dimension: for a token
model, eight spaces of indentation are a repeated whitespace token, and the
vertical alignment between consecutive lines is invisible. Rendering the file
as an image restores the two dimensions the developer actually produced. The
line index becomes the vertical axis, the column index becomes the horizontal
axis, and properties that are implicit in the sequence (indentation depth,
block shape, alignment, blank-line rhythm, line-length distribution) become
explicit geometry. Convolutional architectures are built around exactly this
kind of two-dimensional locality, which makes them a natural model class for
the representation.
...
\cite{ojala2002lbp,maenpaa2005texture,martins2013database}.
...
models by adapting attention mechanisms \cite{vaswani2017attention} to image
patches \cite{dosovitskiy2021vit}.

Transfer learning is particularly useful when a model pretrained on a large
dataset, such as ImageNet \cite{deng2009imagenet}, can be adapted to a smaller
or more specialized dataset. Although
...
Developer feedback should also be calibrated. Poorly framed automated feedback
can be perceived as noise and even reduce communication among developers
\cite{wessel2021dontdisturb}, so a tool should avoid presenting a prediction as
an unquestionable fact. Instead, it should communicate that a file visually
resembles a class, that the confidence is limited, or that the model focused on
particular regions. This is where explainability and interface design meet:
attribution maps, confidence scores, and textual summaries can help developers
decide whether to trust, ignore, or investigate a prediction.

The MinimapSignatures prototype is therefore part of the research strategy: it
allows the project to study how minimap analysis can be delivered in IDEs and
which forms of feedback are plausible for future evaluation.
...
broader XAI literature, including model-specific and model-agnostic methods.
...
comments that describe business rules. Consider a hypothetical assignment such
as \texttt{profitMargin = grossRevenue * 0.2345}: even without knowing the
context, a reader who sees the identifier and the constant together can infer
something about the company's internal calculations. Sending such code to an
external service, even for legitimate analysis, exposes intellectual property
that the organization may not wish to share.
...
Within this argument, the Author objective works as a probe. If a model can
identify the stylistic patterns of a contributor after all identifiers,
literals, and comments have been suppressed, then whatever the minimap captures
comes from indentation habits, spacing conventions, punctuation placement, and
code organization, not from names or values. Author prediction is therefore a
test of the anonymization argument rather than a deployment target: style
surviving suppression is what shows that the model operates on structure alone.
...
predictive performance; this evaluation is part of the planned research.
