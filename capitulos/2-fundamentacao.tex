...
In this project, XAI is used primarily to interpret deep models trained on
minimap images. Integrated Gradients is especially relevant because it attributes
prediction scores to input features by integrating gradients along a path from a
baseline input to the actual input \cite{integratedgradients2017}. When applied
to minimaps, attribution maps can highlight whether the model is using
indentation bands, delimiter regions, dense blocks, headers, boilerplate, or
other visual structures.

These explanations do not prove causality by themselves. A saliency map is an
interpretive aid, not a complete explanation of a model. Nevertheless, XAI helps
connect quantitative results to qualitative understanding. For example, if a
model classifies file Type by focusing on structural bands and delimiter
patterns, that behavior is more consistent with the research hypothesis than if
the model relied exclusively on irrelevant borders or dataset artifacts.
