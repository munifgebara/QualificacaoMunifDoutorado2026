...
\cite{he2016resnet,tan2019efficientnet}.
...
These accuracy values must be read together with imbalance-aware metrics.
Large head classes may dominate global accuracy while minority classes remain
poorly classified. Macro-F1 scores, per-class recall, and confusion matrices
are therefore essential for interpreting these results honestly. Improving
minority-class behavior is one of the central open tasks for the remaining
doctoral work.
...
MinimapSignatures consists of a Python backend (FastAPI with a PyTorch multitask
model), a Visual Studio Code extension, and an IntelliJ IDEA plugin. The backend
exposes a single HTTP endpoint that receives a 128 by 128 base64-encoded
grayscale PNG and returns top-three ranked predictions for three targets: Type,
Project, and Author. A fourth target (quality) is included in the response
contract as a placeholder; no model has been trained for it yet, and it
currently returns mock data. The quality target is reserved for future work.

The model deployed in the backend is a custom multitask convolutional network
with a shared backbone---four successive convolutional blocks that progressively
reduce the spatial dimensions from 128 to 8 pixels---followed by a 256-dimensional
embedding layer and three independent linear classification heads, one per
target. This architecture is distinct from the ResNet-18 and EfficientNet-B0
models evaluated in Study 2; it was designed specifically for simultaneous
multi-target inference in the serving context.

The functional evaluation confirms that the end-to-end workflow operates
correctly: the VS Code extension and IntelliJ IDEA plugin each capture the
active source file, generate an anonymized 128 by 128 minimap locally, encode
it as a base64 PNG, send it to the backend, and display the returned
predictions. The IntelliJ plugin opens a structured Markdown report in a
split-screen editor, while the VS Code extension presents results in a WebView
panel. It is worth noting that the IntelliJ plugin currently captures only the
first 128 lines of the active file; files longer than 128 lines are truncated
before minimap generation.
...
However, no user study has been conducted yet; the current evaluation is
functional and integration-oriented, not a measure of developer usefulness or
perceived benefit.
...
representations was conducted during the development of the approach. The
results showed that predictive performance is largely preserved after
anonymization, with minimal degradation across the evaluated objectives.
This finding has two important implications.

First, it confirms that the discriminative signals captured by the model reside
primarily in structural features---indentation patterns, operator density,
punctuation layout, and block organization---rather than in identifier names or
literal values. The model does not require lexical content to distinguish file
types, project origins, or authorship styles.

Second, it supports the core argument for the approach in industrial contexts.
If anonymization preserves utility while suppressing identifiers, numeric
constants, and string literals, then an organization can send anonymized minimaps
to an external analysis service without exposing intellectual property. The model
receives only structural signals; the business logic embedded in the lexical
layer remains local.

The Author objective plays a specific methodological role in this context. The
fact that author style can be identified even after full lexical suppression is
not a concern---it is evidence. It demonstrates that stylistic patterns
(indentation habits, spacing conventions, operator placement) survive anonymization
and that the model genuinely operates on structure. If the model required lexical
content to identify authors, this argument would fail.

A formal anonymization ablation study remains planned for the thesis. The
preliminary comparison was not systematically documented; the thesis should
compare multiple suppression levels, measure performance degradation curves,
and characterize what information remains inferable from anonymized minimaps
under each strategy.
