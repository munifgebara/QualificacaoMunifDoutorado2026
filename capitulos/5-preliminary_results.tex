...
prevent the current results from being read as final. These limitations are what the research plan of
Chapter~\ref{chapter:research-plan} is designed to resolve.
...
A third study then isolates the anonymization question with a paired design.
...
of the minimap is the histogram of those codes. The histogram is what makes the
representation useful here: it discards where each pattern occurred and retains
only how often each local configuration appears, so two files with the same
...
basis for the study, and the anonymization step was what made their use possible
at all. Processing them produced 42,820 minimap images, organized into 28
...
One implementation detail of the generation step matters for the descriptor. An
...
Anonymization is what allows the approach to be applied to code that cannot
leave an organization, so the claim that it preserves predictive utility carries
a large share of the proposal. Until recently that claim rested on a single
...
The experiment exploits a property of the method that makes an unusually clean
comparison possible. Anonymization is a deterministic per-character map applied
...
two datasets are paired instance by instance. This pairing is what licenses
paired statistical tests, which are considerably more sensitive than comparing
...
Two questions are asked separately, because they are not the same question. The
...
declared margin with a wide safety factor.
...
original one. The honest reading is therefore not that the two representations
are indistinguishable, but that the difference between them is small and, in
this setting, slightly favourable to anonymization.
...
\subsection{What the Transformation Destroys}
...
Placing the two measurements side by side gives the central result of this
section: more than half of the information carried by each pixel is discarded,
and predictive accuracy does not fall. This is direct evidence for the claim on
which the proposal rests, namely that the discriminative signal in a minimap is
structural rather than lexical. It also explains why the surviving 25.9\% is
sufficient, since punctuation and indentation are precisely what encode block
