...
F1-score in both fixed-size and variable-size settings. This result is
...
The data protocol deserves explicit disclosure. Before splitting, classes with
...
The contrast between near-perfect ROC-AUC and accuracies of 86--87\% deserves
an explicit explanation. With roughly one hundred classes evaluated
...
The training pipeline produces per-class classification reports and confusion
matrices for each objective; consolidating macro-F1 and per-class recall into
the results tables is part of the planned evaluation work, and improving
minority-class behavior remains one of the central open tasks.
...
This result changes the research trajectory. The thesis is not limited to a
dataset and model comparison; it also investigates how minimap analysis can
become usable infrastructure for software-engineering workflows. No user study
...
\midrule
LBP study & Minimap texture supports software-feature classification with Random Forest near 93\% F1. & RQ1, RQ2 \\
...
A comparison between anonymized and non-anonymized minimap representations was
carried out informally while the approach was being developed. Predictive
performance stayed close to the non-anonymized baseline across the evaluated
objectives. The comparison was not documented under controlled conditions, so
it is reported here as preliminary evidence rather than as a result, but it
...
Second, it supports the industrial motivation of the approach. If anonymization
preserves utility while suppressing identifiers, numeric constants, and string
literals, an organization can submit anonymized minimaps to an external
analysis service while the business logic embedded in the lexical layer stays
local.

The Author objective has a specific role in this argument. Style remains
identifiable after full lexical suppression, which provides evidence that the
model operates on structural patterns such as indentation habits, spacing
conventions, and operator placement. If author identification depended on
lexical content, anonymization would have eliminated it.
