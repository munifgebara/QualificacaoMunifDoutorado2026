Three studies make up the empirical work completed so far, and they are
reported here in sequence. The first two are ordered from the most controlled to
the most demanding setting: a small, balanced experiment with handcrafted
texture descriptors, and a large-scale experiment with deep convolutional models
over hundreds of thousands of minimaps. The third isolates a single question
that the proposal depends on, whether anonymization costs predictive
performance, and answers it with a paired design and an explicit statistical
statement. The chapter then consolidates what the studies jointly support, maps
that evidence back to the research questions, and is explicit about the
limitations that prevent the current results from being read as final. The
research plan of Chapter~\ref{chapter:research-plan} is built to resolve them.
...
The initial dataset was generated from two large proprietary systems, made
...
Figure~\ref{fig:lbp-histograms} presents average LBP histograms for six of the
seven classes, computed over 1{,}000 minimaps per class; the seventh (the
UnitTest stereotype) is omitted only to keep the six overlaid curves
distinguishable. The figure is
...
reports that quantity alone.\footnote{No per-class or averaged precision-recall
figures were computed for this experiment; overall accuracy is the only figure
of merit available for it. Producing them, from the per-class classification
reports that the pipeline already generates, is part of the consolidation work
described in Chapter~\ref{chapter:research-plan}.} The values should be interpreted as
...
is defined by axioms, independently of any particular architecture: it
...
to replace it. It is a first installment of Activity A2, not its completion;
the remaining scope is stated at the end of the section and in
...
The corpus is the government system used in the first study, in a more recent
version. Files were selected by extension, and only
...
Neither test should be read in isolation. The difference
...
setting, slightly favorable to anonymization.

A plausible explanation is that anonymization does not only discard
information; it also filters. Collapsing letters and digits onto three
intensities removes lexical variation that a texture descriptor cannot exploit
anyway, leaving the structural skeleton more salient. The measurement in the next subsection
...
Paired ablation (Study 3) & Anonymization has no accuracy cost (equivalence within two points, $p = 2.4 \times 10^{-5}$) while discarding 56.7\% of per-pixel entropy. & RQ4 \\
...
favor of the anonymized representation, with a 95\% confidence interval whose
...
for an SVM in a ten-class configuration of the same corpus, a wider class
selection than the balanced seven-class setting reported above
\cite{gebara2025iwssip}, though that figure came from a single uncontrolled
run.
