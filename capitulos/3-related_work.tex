This chapter positions the proposal within the existing literature. It first
documents the mapping strategy used to assemble the corpus of studies, so that
the coverage of the review can be judged and repeated. It then organizes the
retrieved work into families that correspond to the different ways the community
has approached visual and learned analysis of source code, and examines each
family for what it already establishes and for what it leaves unresolved. Those
unresolved points are consolidated at the end of the chapter into the gaps that
this research addresses.

\section{Literature Mapping Strategy}

The related-work analysis is based on a structured literature mapping. The
search strategy combined four logical blocks executed across five major digital
libraries: IEEE Xplore, ACM Digital Library, Scopus, SpringerLink, and Google
Scholar.
...
\textit{(A) AND (B) AND (C) AND (D)}; complementary runs omitted Block D to
cover broader analysis methodologies.

Reference management relied on Zotero for automated metadata harvesting. Study
selection and blinded screening were managed through the Rayyan platform and
performed by two reviewers, the author and an undergraduate research student
(Jamily G. S. Souza), with conflicting assessments resolved by discussion.
Title and abstract screening produced an initial set of 270 candidates.
Because the combined queries returned many works that mention visualization
only incidentally, a second filter compared candidate keywords against 39
structural index terms extracted from a control group of 7 baseline papers
central to the topic, narrowing the pool to 155 articles. Negative filtering
then eliminated secondary studies and out-of-scope formats (survey, review,
mapping study, tutorial, editorial, poster, and works centered on security or
UX), reducing the active set to 96 articles, and full-text inspection with
iterative peer validation produced the final corpus of 56 selected studies.

This chapter does not discuss all 56 studies individually. The corpus was
first clustered into thematic groups, using a TF-IDF-weighted
article-by-term matrix and agglomerative clustering, as a navigation aid; the
groups were then translated into the conceptual families that organize the
following sections: software visualization, machine learning representations
for code, visual code representations, software quality, explainability, and
tool support. The sections below discuss the most representative works of each
family, selected for their proximity to the minimap proposal. The full corpus,
the search strings, and the screening decisions will be packaged as a citable
artifact, as described in Chapter~\ref{chapter:research-plan}.
...
images. Two things change with this move. Dimensionality: token- and
sequence-based models consume code as a one-dimensional stream, whereas the
minimap preserves the two-dimensional layout of lines and columns, in which
indentation, alignment, and block shape are directly visible. And generality:
explicit semantics are lost, but uniformity is gained across many file
...
In this work, the large-scale study (Study 2) evaluated ResNet-18 and
EfficientNet-B0; Vision Transformer configurations (ViT-B/16) were also
exercised in early experimentation runs, but they were not part of the
consolidated evaluation. The MinimapSignatures backend uses a custom
MultiTaskMinimapCNN with a shared convolutional backbone and independent
classification heads per target. A systematic comparison including
transformer-based and hybrid architectures remains planned work.
...
engineering, where non-visual signals are rendered as images for visual
learning: spectrograms have supported music genre classification with both
texture descriptors and neural models
\cite{costa2011music,costa2012music,costa2017cnn}, and similar sound-image
strategies appear in other domains
\cite{nanni2018bird,pereira2019representation}. A domain need not be inherently
visual to benefit from visual machine learning, as long as the transformation
preserves task-relevant patterns; Chapter~\ref{chapter:foundations} argues that
this route is even more direct for source code, which already has a
two-dimensional layout of its own.
...
\section{Granularities of Information in Visual Code Analysis}

Across the surveyed studies, visual representations of code are used to answer
questions at three granularities: what kind of artifact a file is, which
codebase or component family it belongs to, and which stylistic conventions it
follows. Work on visual file classification and structural stereotypes
addresses the first granularity; work on architectural similarity and
repository conventions addresses the second; stylometric and
authorship-oriented analyses address the third.

The three prediction objectives adopted in this work (Type, Project, and
Author) were chosen to probe these granularities with a single representation.
The objectives are not themselves findings of the mapping; they operationalize,
over minimap images, the kinds of questions that the surveyed studies ask over
other representations of source code.

\section{Confidentiality and Code Representations}

One dimension of the mapping is treated separately because it anchors the
central motivation of this proposal. The mapping did not identify studies that
design a code representation primarily to suppress proprietary lexical content
while remaining useful as input for machine learning. Related concerns exist in
adjacent areas, but with different goals. Code obfuscation transforms programs
to hinder reverse engineering, protecting the shipped artifact rather than
producing an analyzable research representation. Secret-scanning tools remove
credentials from repositories but leave identifiers, literals, and comments
intact. Privacy-preserving machine learning, such as federated learning,
protects training data at the protocol level rather than at the representation
level. None of these lines produces a compact, language-light representation in
which identifiers, literals, and comments are suppressed by construction.

To the best of the current mapping, therefore, the claim that anonymized
minimaps enable analysis of codebases that cannot be shared in raw form is not
directly addressed by prior work. This absence strengthens the originality of
the proposal, but it also means the claim must be supported by evidence
produced within this work, in particular the anonymization ablation planned in
...
constructing a language-aware representation from the one-dimensional character
stream, it renders source artifacts into a two-dimensional visual space,
keeping the page layout that linear token streams flatten away. This makes the
representation weaker semantically but broader operationally. It can include code, markup, configuration, scripts, templates,
...
The literature also reveals open problems that the thesis must address. Three
gaps recur across the surveyed studies:

\paragraph{Detachment from developer tooling.}
Visual pipelines for source code machine learning remain largely disconnected
from real-time developer workflows. Few studies provide plugin infrastructure
able to capture, anonymize, and transmit code from a live IDE session, and
offline accuracy metrics say little about the friction of live software
engineering, where files are often incomplete or in the middle of an edit.

\paragraph{Lack of user-centered evaluation.}
Evidence about usability, cognitive load, and developer trust in visual AI
feedback is scarce. Few studies observe real developers interacting with such
tools, so it is not yet possible to say whether minimap-based feedback is
useful, interpretable, or trustworthy in practice.

\paragraph{The single-file scale barrier.}
Current prototypes operate on isolated, single-file representations at a fixed
visual scale. Support for multi-file modules, project-level analysis, adaptive
multi-scale inputs, and user-driven retraining is rare in the surveyed work.
This restricts minimap analysis to tasks that fit within one file.
...
\begin{itemize}
    ...
    \item \textit{Confidentiality and leakage}: anonymization removes lexical
    content but is not a formal guarantee. Generated files, framework templates,
    or distinctive repository conventions may remain identifiable. Stronger
    suppression modes must be evaluated.
