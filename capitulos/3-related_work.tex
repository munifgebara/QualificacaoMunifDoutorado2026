...
A significant finding from recent literature is the transition toward deep
visual computing for minimap-based analysis. Early approaches relied on
handcrafted texture descriptors such as Local Binary Patterns. More recent work
demonstrates an increasing adoption of end-to-end computer vision architectures:
Convolutional Neural Networks (CNNs), residual learning backbones (ResNet), and
Vision Transformers (ViTs) are now used to process anonymized minimap images
directly, treating structural layouts as machine-readable imagery without
requiring lexical parsing. The adoption of ViTs is particularly relevant because
their patch-based attention mechanism aligns naturally with the spatial
organization of minimap regions such as indentation bands, delimiter zones, and
block boundaries.
...
adopted in this doctoral project.
