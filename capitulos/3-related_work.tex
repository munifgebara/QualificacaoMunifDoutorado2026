...
doctoral project treats minimaps as visual signatures that can be processed by
...
One of the long-term goals of this doctoral project is to extend minimap-based
...
The literature also reveals open problems that the thesis must address.

The first open problem is representation fidelity. Visual representations may
discard important semantics. A minimap can preserve layout and symbols, but it
does not explicitly encode type hierarchies, API usage, or data flow. The thesis
must therefore identify which tasks are appropriate for minimaps and which tasks
require complementary representations.

The second open problem is privacy and leakage. Anonymization suppresses lexical
content, but visual structure can still reveal information. A generated file, a
framework template, or a distinctive repository convention may be identifiable.
The thesis must discuss this honestly and evaluate stronger anonymization modes.

The third open problem is generalization. Strong performance inside one corpus
does not guarantee generalization to other ecosystems. Public open-source
projects may not represent proprietary systems. Large projects may dominate
training. Generated files may inflate results. The thesis must use careful
splits, ablations, and per-class analysis.

The fourth open problem is developer interpretation. A classifier that reports
``Project A, 92\% confidence'' may not help a developer if the meaning of the
prediction is unclear. Explanations, uncertainty, and actionable framing are
necessary. Tool evaluation must therefore include not only whether the system
runs, but whether its feedback can be understood and used responsibly.
