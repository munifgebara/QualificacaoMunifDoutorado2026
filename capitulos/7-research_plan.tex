...
As reported in Chapter~\ref{chapter:preliminary-results}, an informal comparison
between anonymized and non-anonymized minimaps suggested that anonymization
costs little predictive performance. That comparison was not documented under
controlled conditions, so Activity A2 will produce a rigorous study comparing
several suppression levels:
...
macro-F1 per objective; accuracy and per-class recall are secondary. The
declared expectation, based on the informal comparison, is that the baseline
anonymization costs at most two percentage points of macro-F1 relative to the
non-anonymized rendering. A larger drop will be reported as a negative result
and will trigger a revision of the confidentiality claims made in this
proposal. Results for all levels will be reported regardless of outcome, and
...
Model comparison & Evaluate additional CNN backbones, transformer-based variants (resuming the early ViT-B/16 runs), and the multitask architecture under identical repository-aware splits. & Stronger benchmark of architecture choices. \\
...
stored externally. The informal evidence discussed in
Chapter~\ref{chapter:preliminary-results} suggests that this suppression costs
little predictive performance; Activity A2 will quantify the trade-off.
...
with a versioned DOI (10.5281/zenodo.16929672); the training pipelines, the
backend, and the IDE plugins are available in public repositories; and the
...
Anonymization removes too much signal & Models lose predictive power. & Informal evidence suggests low risk; the Activity A2 ablation will quantify it. \\
...
September--December 2026 & Refine dataset catalog (A1); run the anonymization ablation (A2) and imbalance-aware experiments (A3). \\
January--April 2027 & Evaluate additional architectures and resolutions (A4); consolidate explanation protocols; begin writing methodology and results chapters. \\
May--August 2027 & Quality or architectural-deviation pilot (A5); extend MinimapSignatures with multi-file analysis and explanation overlays. \\
...
months available until the expected conclusion at the end of 2027.
