This chapter introduces the research. It describes the setting that motivates
the work, namely the difficulty of analyzing large and heterogeneous codebases
whose content often cannot be shared with external analysis services, and it
states the problem that follows from this setting. It then delimits the gap left
by existing code representations, formulates the hypothesis that source code
minimaps retain enough structural information to support analysis after lexical
anonymization, and derives the research questions, objectives, and expected
contributions that organize the remainder of the document.
...
minimaps}, compact image-like representations generated from source files.

The proposal rests on two representational commitments. The first is a change
of dimensionality. Source code is stored and almost always analyzed as a
one-dimensional sequence: parsers, token-based models, and sequence models all
consume code as a linear stream of characters or tokens. Developers, however,
do not read code as a stream. They read it as a two-dimensional page, in which
indentation forms contours, blocks have shapes, blank lines create rhythm, and
the distribution of line lengths gives each file a silhouette. The minimap
makes this two-dimensional structure the primary signal: each character becomes
a pixel and each line a row, so the layout that a sequence model treats as
incidental whitespace becomes geometry that a vision model can exploit
directly, while lexical content can be anonymized.

The second commitment concerns the consumer of the visualization. The idea is
inspired by the minimap widgets that developers already know from modern
editors, and software visualization has historically produced images for human
inspection. In this research, the minimap is not a user-interface element for
manual navigation: it is a machine-readable visual abstraction, produced to be
processed by texture descriptors, computer vision models, deep learning
architectures, and explainable artificial intelligence techniques. The human
remains the consumer of the resulting analysis, but the image itself is made
for the model.
...
classifiers to detect software features, with Random Forest reaching an
accuracy near 93\% in the evaluated setting \cite{gebara2025iwssip}. A second study
...
many existing representations either require language-specific processing or
retain lexical content that exposes business logic, proprietary constants, and
strategic domain knowledge, which makes external analysis impractical for
industrial codebases.
\end{quote}

Traditional static analysis is effective when the target language, framework,
and rule set are known, but such tools typically require parsers, language-specific
rules, and expert configuration. Software visualization approaches expose
structures to human analysts but are not designed as machine-readable
representations for large-scale classification. Machine learning representations
for code (tokens, abstract syntax trees, graphs, pretrained language models) are
effective but retain lexical content. Sending a raw source file to an external
service exposes identifier names, numeric constants, formulas, and comments
that may encode business rules, which is often enough for an industrial
organization to rule the tool out. This limits the practical applicability of
many analysis approaches precisely in the contexts where they would be most
valuable.

The project therefore asks whether a visual representation of source files can
serve as an intermediate abstraction: simple enough to generate across many
languages and file types, compact enough to support large datasets, and able to
retain discriminative signals while suppressing the lexical content, so that
business-sensitive information never leaves the local environment. Source code
minimaps are proposed as this abstraction.
...
Despite these advances, four gaps remain relevant for this work.
First, the two research communities approach the visual dimension of code from
opposite sides and leave a space between them: visual tools are primarily
designed for human inspection rather than for systematic machine learning over
large corpora, while machine learning representations consume code as linear
sequences, trees, or graphs and do not treat the two-dimensional layout of the
source page as a first-class signal. Second, many learning-based
representations depend on language-specific preprocessing or retain lexical
content, i.e., identifiers, constants and comments, that exposes business logic
and intellectual property, limiting applicability in industrial settings. Third,
there is still limited empirical evidence about how much structural information
compact minimap images preserve across many projects, languages, and prediction
objectives, and whether anonymization can suppress sensitive lexical content
without substantially degrading predictive performance. Fourth, minimap-based
...
\begin{quote}
Anonymized source code minimaps, understood as two-dimensional projections of
source files intended for machine consumption, preserve enough structural and
visual information to support scalable and language-agnostic software analysis,
while reducing dependence on language-specific parsers and expert-defined
rules.
\end{quote}
...
against all possible structural inferences. The claim is narrower: the
character-level anonymization adopted here suppresses the lexical layer that
carries direct intellectual property (identifiers, constants, string literals,
and comments), while the remaining structural signals retain enough
discriminative power for the targeted analysis tasks. Preliminary evidence for
this boundary is discussed in Chapter~\ref{chapter:preliminary-results}. Third, it does not claim that a single visual
...
representation for a family of analysis tasks, especially in contexts where
lexical exposure to external systems is undesirable.
...
The general objective of this work is to define, evaluate, and
...
\midrule
Representation & Source code minimaps: compact two-dimensional visual signatures produced for model consumption. & Supported by two studies \\
Anonymization & Character-to-pixel mapping that suppresses lexical content while preserving layout signals. & Measured on one system; broader ablation planned \\
Datasets & 685,906-image corpus from 100 projects across 10 languages; archived on Zenodo. & Available \\
Modeling & LBP + shallow classifiers (Study 1); ResNet-18 and EfficientNet-B0 (Study 2); custom multitask CNN (Stage 3). & Evaluated in two studies \\
...
each file can be rendered independently. File-level analysis is also a
...
The project also rests on assumptions that mirror the hypothesis boundaries
stated earlier. Visual structure is assumed to be meaningful even when lexical
content is suppressed: software artifacts are not random text, and formatting
conventions, indentation, block delimiters, annotation placement, and generated
sections produce visual patterns. This assumption must be tested empirically,
which is why anonymization is treated as part of the representation rather than
as an optional post-processing step. Likewise, minimaps are assumed to be
useful as a first, language-light representation even though deeper tasks still
require language-specific analysis: a minimap may indicate that a file
resembles a test class, but it cannot prove that the tests assert meaningful
behavior. The intended use is decision support, with human judgment and
complementary tools remaining central.

Finally, the project assumes that practical adoption depends on tool support.
An offline dataset and model demonstrate feasibility, but a
software-engineering technique becomes more meaningful when connected to actual
workflows. MinimapSignatures is included in the scope of the doctorate for this
reason: it provides a path from representation to developer interaction and a
platform for future studies about usefulness, trust, and explanation design.
