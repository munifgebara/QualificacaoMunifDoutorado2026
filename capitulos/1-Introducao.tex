...
powerful but retain lexical content. Sending a raw source file containing
identifiers such as \texttt{profitMargin}, formulas such as
\texttt{(sin(x) + cos(y)) * 23.45}, or comments describing business rules to an
external service may expose intellectual property that industrial organizations
cannot share. This limits the practical applicability of many analysis tools
precisely in the contexts where they would be most valuable.
...
against all possible structural inferences. The claim is more specific and more
useful: the character-level anonymization adopted here suppresses the lexical
layer that carries direct intellectual property---identifiers, constants, string
literals, and comments---while the remaining structural signals retain enough
discriminative power for the targeted analysis tasks. A preliminary comparison
between anonymized and non-anonymized representations confirms that performance
degradation is minimal. Third, it does not claim that a single visual
